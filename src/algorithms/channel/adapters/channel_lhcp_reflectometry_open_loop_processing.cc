 ...
 * -------------------------------------------------------------------------
 */
#include "channel_lhcp_reflectometry_open_loop_processing.h"
//#include <memory>
...
#include "open_loop_processing_interface.h"
...
// Constructor
Channel_LHCP_Reflectometry_Open_Loop_Processing::Channel_LHCP_Reflectometry_Open_Loop_Processing(ConfigurationInterface *configuration, unsigned int channel,
        std::shared_ptr<GNSSBlockInterface> pass_through, std::shared_ptr<AcquisitionInterface> acq,
        std::shared_ptr<TrackingInterface> trk, std::shared_ptr<TelemetryDecoderInterface> nav, std::shared_ptr<OpenLoopProcessingInterface> olp,
        std::string role, std::string implementation, boost::shared_ptr<gr::msg_queue> queue)
{
    ...
    olp_->set_channel(channel_);

    gnss_synchro_.Channel_ID = channel_;
    acq_->set_gnss_synchro(&gnss_synchro_);
    trk_->set_gnss_synchro(&gnss_synchro_);
    olp_->set_gnss_synchro(&gnss_synchro_);


    nbCorr_ = configuration->property("Number of correlators (reflected way)",3);
...
Channel_LHCP_Reflectometry_Open_Loop_Processing::~Channel_LHCP_Reflectometry_Open_Loop_Processing() = default;

void Channel_LHCP_Reflectometry_Open_Loop_Processing::connect(gr::top_block_sptr top_block)
{
    if (connected_)
        {
    ...
    top_block->connect(pass_through_->get_right_block(), 1, trk_->get_left_block(), 1);
    DLOG(INFO) << "pass_through_ -> tracking";
    top_block->connect(trk_->get_right_block(), 0, nav_->get_left_block(), 0);

    top_block->connect(trk_->get_right_block(), 0, olp_->get_left_block(), 2*(nbCorr_+3));
    DLOG(INFO) << "tracking -> telemetry_decoder";
    for(int i=0;i<2*(nbCorr_+3); i++){
        top_block->connect(trk_->get_right_block(), i+1, olp_->get_left_block(),i);
    }
    // Message ports
...
void Channel_LHCP_Reflectometry_Open_Loop_Processing::disconnect(gr::top_block_sptr top_block)
{
    if (!connected_)
        {
    ...
    top_block->disconnect(pass_through_->get_right_block(), 1, trk_->get_left_block(), 1);
    top_block->disconnect(trk_->get_right_block(), 0, nav_->get_left_block(), 0);
    for(int i=0;i<2*(nbCorr_+3); i++){
        top_block->disconnect(trk_->get_right_block(), i+1, olp_->get_left_block(),i);
    }
    pass_through_->disconnect(top_block);
    acq_->disconnect(top_block);
    trk_->disconnect(top_block);
    nav_->disconnect(top_block);
    olp_->disconnect(top_block);
    connected_ = false;
}


gr::basic_block_sptr Channel_LHCP_Reflectometry_Open_Loop_Processing::get_left_block()
{
    return pass_through_->get_left_block();
}


gr::basic_block_sptr Channel_LHCP_Reflectometry_Open_Loop_Processing::get_right_block()
{
    return nav_->get_right_block();
}


void Channel_LHCP_Reflectometry_Open_Loop_Processing::set_signal(const Gnss_Signal& gnss_signal)
{
...
void Channel_LHCP_Reflectometry_Open_Loop_Processing::start_acquisition()
{
